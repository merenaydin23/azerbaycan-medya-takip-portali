...
class HurriyetAdapter(BaseAdapter):
    def __init__(self):
        super().__init__(
            source_id="hurriyet",
    ...
    def fetch_latest_news(self) -> list:
        ...
        # Fallback Scraper
        scrape_urls = ["https://www.hurriyet.com.tr/gundem/", "https://www.hurriyet.com.tr/dunya/"]
        for url in scrape_urls:
            html = self.fetch_url(url)
            if not html:
                continue
            soup = BeautifulSoup(html, "html.parser")
            cards = soup.select(".card, .news-box, a[href*='hurriyet.com.tr/gundem/'], a[href*='hurriyet.com.tr/dunya/']")
            for card in cards[:20]:
                ...
                title_tag = card.find(["h2", "h3", "h4", "h5"]) or link_tag
